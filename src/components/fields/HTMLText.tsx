...
const DEFAULT_TEXT_DECORATIONS: TextDecorationControl[] = [
    "underline",
    "strike",
];
const DEFAULT_LISTS: ListControl[] = ["bulletList", "orderedList"];
const DEFAULT_TEXT_ALIGN: TextAlignControl[] = [
    "left",
    "center",
    "right",
    "justify",
...
const textAlignOrder: TextAlignControl[] = [
    "left",
    "center",
    "right",
    "justify",
...
const toolbarIcons = Object.entries(toolbarIconModules).reduce<
...
function normalizeToolbar(buttons: HTMLTextButton[]): NormalizedToolbar {
    ...
    for (const button of buttons) {
        ...
        if ("backgroundColors" in button) {
            toolbar.backgroundColors = normalizeColorOptions(
        ...
        if ("textStyle" in button) {
            toolbar.textStyle.mode = "select";
            for (const item of button.textStyle.filter(
                (value): value is TextStyleControl =>
        ...
        if ("textDecoration" in button) {
            toolbar.textDecoration.mode = "select";
            for (const item of button.textDecoration.filter(
                (value): value is TextDecorationControl =>
        ...
        if ("lists" in button) {
            toolbar.lists.mode = "select";
            for (const item of button.lists.filter(
                (value): value is ListControl => listOrder.includes(value),
            )) {
                uniquePush(toolbar.lists.items, item);
            }
        }
        ...
        if ("fontWeights" in button) {
            toolbar.fontWeights = [
                ...new Set(button.fontWeights.map((value) => `${value}`)),
            ];
        }
        ...
        if ("textAlign" in button) {
            toolbar.textAlign.mode = "select";
            for (const item of button.textAlign.filter(
                (value): value is TextAlignControl =>
    ...
    toolbar.textStyle.items = sortByOrder(
        toolbar.textStyle.items,
        textStyleOrder,
    );
    toolbar.textDecoration.items = sortByOrder(
        toolbar.textDecoration.items,
        textDecorationOrder,
    );
    toolbar.lists.items = sortByOrder(toolbar.lists.items, listOrder);
    toolbar.textAlign.items = sortByOrder(
        toolbar.textAlign.items,
        textAlignOrder,
    );
...
function HTMLTextComponent({
    ...
    const getControlLabel = (name: AtomicButton) => {
        ...
        if (Icon) {
            return <Icon className='text-4.5' />;
        }
    ...
    return (
        <div>
            <div className='mb-2 flex flex-wrap items-center gap-2 editor-controls'>
                {toolbar.headings && toolbar.headings.length > 0 && (
                    <Select
                        data-grow
                        placeholder='Niveau'
                        value={
                            editorState.currentHeading
                                ? `${editorState.currentHeading}`
                                : "no-heading"
                        }
                        options={[
                            {
                                value: "no-heading",
                                render: () => translation("richTextHeading"),
                            },
                            ...toolbar.headings.map((level) => ({
                                value: level,
                                render: () => `H${level}`,
                            })),
                        ]}
                ...
                {toolbar.fontFamilies && toolbar.fontFamilies.length > 0 && (
                    <Select
                        data-grow
                        value={
                            toolbar.fontFamilies.find(
                                (font) =>
                                    font === editorState.currentFontFamily,
                            ) ?? ""
                        }
                ...
                {toolbar.fontSizes && toolbar.fontSizes.length > 0 && (
                    <Select
                        ...
                        options={[
                            {
                                value: "",
                                render: () => translation("defaultFontSize"),
                            },
                            ...toolbar.fontSizes.map((s) => ({
                                value: s,
                                render: () => s,
                            })),
                        ]}
                        value={
                            toolbar.fontSizes.find(
                                (s) => s === editorState.currentFontSize,
                            ) ?? ""
                        }
                ...
                {toolbar.fontWeights && toolbar.fontWeights.length > 0 && (
                    <Select
                        data-grow
                        value={
                            toolbar.fontWeights.find(
                                (weight) =>
                                    weight === editorState.currentFontWeight,
                            ) ?? ""
                        }
                        ...
                        options={[
                            {
                                value: "",
                                render: () => translation("defaultFontWeight"),
                            },
                ...
                {toolbar.textStyle.items.length > 0 &&
                    toolbar.textStyle.mode === "buttons" && (
                ...
                {toolbar.textDecoration.items.length > 0 &&
                    toolbar.textDecoration.mode === "buttons" && (
                        <div className='flex items-center gap-1'>
                ...
                {toolbar.textDecoration.items.length > 0 &&
                    toolbar.textDecoration.mode === "select" && (
                        <Select
                            ...
                            options={toolbar.textDecoration.items.map(
                                (item) => ({
                                    value: item,
                                    render: () => controlTitles[item],
                                }),
                ...
                {toolbar.lists.items.length > 0 &&
                    toolbar.lists.mode === "buttons" && (
                        <div className='flex items-center gap-1'>
                ...
                {toolbar.backgroundColors &&
                    toolbar.backgroundColors.length > 0 && (
                                        ...
                                        render: () => {
                                            const selected =
                                                (toolbar.backgroundColors?.find(
                                                    (color) =>
                                                        color.value ===
                                                        editorState.currentBackgroundColor,
                                                )?.value ?? "auto") == "auto";
                                            return (
                                                <div className='flex items-center w-full gap-2'>
                                                    <span
                ...
                {toolbar.textAlign.items.length > 0 &&
                    toolbar.textAlign.mode === "buttons" && (
                        <div className='flex items-center gap-1'>
                ...
                {toolbar.textAlign.items.length > 0 &&
                    toolbar.textAlign.mode === "select" && (
                        <Select
                            data-grow
                            value={
                                toolbar.textAlign.items.find(
                                    (item) =>
                                        item === editorState.currentTextAlign,
